...
OUTPUT_SEARCH_FILENAME="shodan.search.out.${domain}.$(date +'%Y_%m_%dT%H_%M_%S')"
...
#run:
shodan download --limit -1 "${OUTPUT_RECON_FOLDER}/${OUTPUT_SEARCH_FILENAME}" ${domain}
#compress output (saving space ftw!)
gunzip -f "${OUTPUT_RECON_FOLDER}/${OUTPUT_SEARCH_FILENAME}.json.gz"
bzip2 -z "${OUTPUT_RECON_FOLDER}/${OUTPUT_SEARCH_FILENAME}.json"
