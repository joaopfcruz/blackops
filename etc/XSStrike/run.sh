#!/bin/bash
#This script runs XSStrike tool
...
OUTPUT_FILENAME="XSStrike.out.${sanitized_url::64}.$(date +'%Y_%m_%dT%H_%M_%S').txt"
...
#run:
python3 bin/xsstrike.py -u "${url}" --skip --blind > "${OUTPUT_EXPLOITING_FOLDER}/${OUTPUT_FILENAME}"
