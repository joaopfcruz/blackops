#!/bin/bash
#This script runs droopescan tool
...
OUTPUT_FILENAME="droopescan.out.${sanitized_url::64}.$(date +'%Y_%m_%dT%H_%M_%S').json"
...
#run:
droopescan scan -u ${url} -e a -o json --hide-progressbar > "${OUTPUT_SCANNING_FOLDER}/${OUTPUT_FILENAME}"
