#!/bin/bash
#This script runs gobuster tool
#arguments <WORKSPACE> <URL> <DIR TESTING LIST>

usage() { echo "Usage: $0 -w <workspace> -u <url> -l <testing list>" 1>&2; exit 0; }

while getopts ":w:u:l:" flags; do
	case "${flags}" in
		w)
			workspace=${OPTARG}
			;;
...
OUTPUT_FILENAME="gobuster.out.${sanitized_url::64}.$(date +'%Y_%m_%dT%H_%M_%S').txt"

if [ -z "${workspace}" ] || [ -z "${url}" ] || [ -z "${wordlist}" ]; then
	usage
fi
...
#run:
gobuster dir -x php -r --random-agent -z -q -w "${wordlist}" -o "${OUTPUT_SCANNING_FOLDER}/${OUTPUT_FILENAME}" -u ${url}
