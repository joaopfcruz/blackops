...
OUTPUT_SEARCH_FILENAME="censys.search.out.${domain}.$(date +'%Y_%m_%dT%H_%M_%S').json"
OUTPUT_SUBDOMAINS_FILENAME="censys.subdomains.out.${domain}.$(date +'%Y_%m_%dT%H_%M_%S').json"
...
#run:
censys search "${domain}" > "${OUTPUT_RECON_FOLDER}/${OUTPUT_SEARCH_FILENAME}"
#compress output (saving space ftw!)
bzip2 -z "${OUTPUT_RECON_FOLDER}/${OUTPUT_SEARCH_FILENAME}"

censys subdomains "${domain}" > "${OUTPUT_RECON_FOLDER}/${OUTPUT_SUBDOMAINS_FILENAME}"
#compress output (saving space ftw!)
bzip2 -z "${OUTPUT_RECON_FOLDER}/${OUTPUT_SUBDOMAINS_FILENAME}"
