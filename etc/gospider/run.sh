...
OUTPUT_FILENAME="gospider.out.${domain}.$(date +'%Y_%m_%dT%H_%M_%S').json"
...
#run:
gospider -s "{domain}" -u web -t 10 --subs --other-source --include-subs --sitemap --robots -q --json > "${OUTPUT_SCANNING_FOLDER}/${OUTPUT_FILENAME}"
