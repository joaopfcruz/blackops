...
OUTPUT_FILENAME="gospider_urlessd.out.${sanitized_url::64}.$(date +'%Y_%m_%dT%H_%M_%S').json"
...
#run:
gospider -s ${url} -u web -t 16 --subs --other-source --include-subs --sitemap --robots -q | python3 ${BLACKOPS_HOMEDIR}/etc/gospider/urless/urless.py | egrep -v "^\[url\].*" > "${OUTPUT_SCANNING_FOLDER}/${OUTPUT_FILENAME}"
